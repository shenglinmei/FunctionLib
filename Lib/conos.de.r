...
#r1=appendSpecifisityMetricsToDE.P2(de1[[n]], group, n, tp2$counts[nname,])

#de.df=de1[[n]]
#cluster.id=n
#clusters=group
#p2.counts=tp2$counts[nname,]
appendSpecifisityMetricsToDE.P2=function (de.df, clusters, cluster.id, p2.counts, low.expression.threshold = 0, 
          append.auc = FALSE) 
{
  
  de.df$pvals=dnorm(de.df$Z)
  de.df$Gene=rownames(de.df)
  de.df$PAdj=p.adjust(de.df$pvals)
  cluster.mask <- setNames(clusters == cluster.id, names(clusters))
  gs=intersect(de.df$Gene,colnames(p2.counts))
